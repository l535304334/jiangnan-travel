...
> 版本：v2.0 | 总任务数：30 | **状态：30/30 全部完成**（Release 1.0，2026-07-08）
> Phase 1-5 为已固化的执行记录；面向未来的规划见文末「后续 Roadmap」。
...
## 后续 Roadmap（非承诺，按价值排序）

> Release 1.0 之后的候选方向。已知问题与技术债的完整清单见
> [../项目开发总结报告.md](../项目开发总结报告.md)「已知问题与后续方向」与
> [UPGRADE_2026-07-26.md](UPGRADE_2026-07-26.md)「后续可选优化」。

| 优先级 | 方向 | 说明 |
|--------|------|------|
| P1 | 真实支付渠道接入 | 当前为模拟支付网关（幂等/重试/状态机已就绪），接入微信/支付宝沙箱即可闭环 |
| P1 | 通知渠道落地 | 短信验证码当前为日志打印（`UserServiceImpl` 已留 TODO），接入短信服务商 |
| P2 | `@CurrentUser` 参数解析器 | 收敛 17 个控制器中重复的 principal 提取样板代码 |
| P2 | 监控接入 | Actuator/Prometheus 端点已暴露，补 Grafana 面板与告警规则 |
| P2 | 死代码清理 | `QuoteWall.vue`（0 引用）等待 `git rm` |
| P3 | 前端 TypeScript 迁移 | 65 个源文件的渐进式迁移 |
| P3 | 多城市扩展 | 计费规则/地标数据按城市维度配置化 |
| P3 | Element Plus 按需引入 | 进一步压缩首屏体积（当前依赖 manualChunks 分包缓解） |
