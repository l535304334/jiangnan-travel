...
| [AI_CAPABILITY_REPORT.md](AI_CAPABILITY_REPORT.md) | AI 开发环境能力报告 — 模型/Skills/MCP/Agent 能力矩阵 |
...
│       ├── config/                   # 8 个配置类（CORS、Security、Cache、RateLimit、Knife4j 等）
│       ├── security/                 # JwtUtil、JwtAuthFilter、SecurityConfig
│       ├── controller/               # 22 个 Controller → 80+ API 端点
│       ├── service/ + impl/          # 22 个 Service 接口 + 22 个实现
│       ├── entity/                   # 28 个实体 + BaseEntity
│       ├── mapper/                   # 28 个 MyBatis-Plus Mapper
│       ├── dto/                      # 25 个请求 DTO
│       ├── vo/                       # 16 个响应 VO
│       └── websocket/                # 4 个实时通信端点（司机位置/订单追踪/通知/配置）
│
├── jiangnan-travel-web/              # 前端 Vue 3 项目
│   ├── package.json                  # 6 个运行依赖 + 2 个构建依赖
│   ├── vite.config.js                # Vite 5 配置（代理 /api → 8080）
│   └── src/
│       ├── api/                      # 13 个 API 模块（封装 Axios + 拦截器）
│       ├── components/               # AiChatFloat、AmapView 公共组件
│       ├── composables/              # 3 个组合式函数（POI搜索/反馈/验证码）
│       ├── views/                    # 38 个页面视图（乘客/司机/管理 三端）
│       ├── router/                   # 路由配置 + 三层鉴权守卫
│       ├── stores/                   # Pinia 状态管理
...
├── docs/spec/                        # 功能规格说明
│
├── docs/sql/                         # SQL 脚本（init.sql、indexes.sql 等 5 个）
│
├── AGENTS.md                         # AI 代理总入口
...
| 模块 | 路径 | 端点数 | 说明 |
|---|---|---|---|
| 用户管理 | `/api/user` | 6 | 验证码、注册、登录、个人信息 |
| 司机管理 | `/api/driver` | 6 | 登录、注册、状态、位置、收入 |
| 司机订单 | `/api/driver/order` | 6 | 接单、到达、出发、完成、附近订单、推荐 |
| 订单管理 | `/api/order` | 9 | 估价、创建、列表、详情、取消、支付、评价、分享、重下 |
| 优惠券 | `/api/coupon` | 3 | 列表、我的、领取 |
| 用户地址 | `/api/user/address` | 3 | 列表、新增、删除 |
| 活动 | `/api/campaign` | 4 | 列表、详情、领券、管理 |
| VIP | `/api/vip` | 5 | 等级、购买、续费、我的会员 |
| 班线 | `/api/bus-line` + `/api/bus-schedule` | 6 | 线路、时刻表、购票 |
| 发票 | `/api/invoice` | 4 | 申请、列表、详情、取消 |
| 支付 | `/api/payment` | 2 | 创建支付、查询状态 |
| 管理后台 | `/api/admin` | 15+ | 大屏、用户/司机/订单/告警/车型/活动/VIP/班线管理 |
| 城市地标 | `/api/landmark` | 2 | 列表、搜索 |
| AI 服务 | `/api/ai` | 5 | 对话、流式对话、会话、推荐目的地 |
| 消息通知 | `/api/notification` | 4 | 列表、未读数、标记已读 |
| 安全风控 | `/api/safety` | 1 | 安全预警 |
| 文旅数据 | 多路径 | 3 | 热点、城市语录、常走路线 |
| AI 分析 | `/api/admin/ai` | 1 | 数据分析 |

> 完整 API 文档见 [ARCHITECTURE.md](ARCHITECTURE.md#五api-接口清单74-个端点) 第五章。
...
| AI | `t_ai_chat_log` |
| 客运 | `t_schedule_route`, `t_schedule_order` |
...
| 动态加价 | `SurgePricingServiceImpl` | 供需比 1.00~1.50 系数动态调价 |
...
| 乘客 | 测试手机号见 `sql/test_accounts.sql` |
