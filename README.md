...
> 以下截图从三个维度展示项目质量：代码质量（状态机设计）、业务完整度（VIP 体系）、安全意识（风控系统）。

| 维度 | 截图 | 说明 |
|------|------|------|
| 代码质量 | ![OrderStatus 状态机](docs/screenshots/11-order-status-enum.png) | `OrderStatus.java` — 订单状态机，10 状态 + 15 条合法流转路径，枚举集中管理，18 个单元测试全覆盖 |
| 业务完整度 | ![VIP 会员中心](docs/screenshots/12-vip-center.png) | 乘客端 VIP 会员中心 — 多档会员等级、成长值、积分权益、续费优惠 |
| 安全意识 | ![风控告警](docs/screenshots/13-risk-alerts.png) | 管理后台风控告警 — 下单频率、深夜跨城、疲劳驾驶检测，三级告警分类 |
